...
---@class OtherItemTab : BaseTab
OtherItemTab = {}
setmetatable(OtherItemTab, { __index = BaseTab })
OtherItemTab.__index = OtherItemTab

function OtherItemTab:New(holder)
    local config = {
        tabName = "Other Items",
        tabNameHandle = "UCT_OtherItemTab_Label",
        idPrefix = "OtherItem",
        fetchMessage = SMS.FetchOtherItems,
        searchLabel = "Search Other Items:",
        searchLabelHandle = "UCT_SearchOtherItems_Label",
        noItemsText = "No other items found.",
        maxTableWidth = 5,
    ...
    setmetatable(instance, OtherItemTab) -- Re-set metatable to the child class

    SMS.FetchOtherItemModNames:SendToServer({ ID = USERID })

    return instance
end

function OtherItemTab:DrawGrid()
    UI_Utils.CreateItemGrid(self.MainArea, self.Items, {
        ...
        renderItem = function(cell, uuid, data)
            ...
            if fullName then
                ...
                local itemButton = cell:AddImageButton("##OtherItem" .. uuid, icon, {100*ViewPortScale, 100*ViewPortScale})
                cell:AddText(name)
                ...
                InfoPopup:AddInfo(popup, data, { { key = "id", label = "ID" }, { key = "fullName", label = "Name" } })
            end
        end
    })
end
